...
class OrderDetailBloc implements Bloc {
  ...
  // --- Font size mapping for sunmi_printer_plus v4.x (fontSize is an int)
  // Feel free to tweak these numeric values to match visual size on your device.
  static const int _fsXS = 12;
  static const int _fsSM = 18;
  static const int _fsMD = 24; // default medium
  static const int _fsLG = 36;
  static const int _fsXL = 48;
  ...
  printInvoice() async {
    ...
    try {
      ...
      await SunmiPrinter.printText(
          ...
          style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true));
      await SunmiPrinter.printText(
      ...
      if (orderDetailsPojo.userTakenType != 2) {
        ...
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: languages.customer, width: 15, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true)),
          SunmiColumn(text: ' ', width: 1, style: SunmiTextStyle(fontSize: _fsMD, bold: true)),
          SunmiColumn(text: languages.driver, width: 15, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true)),
        ]);
        // print data row (names) with normal MD style
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: orderDetailsPojo.customerName, width: 15, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: ' ', width: 1, style: SunmiTextStyle(fontSize: _fsMD)),
          SunmiColumn(text: orderDetailsPojo.deliveryPersonName, width: 15, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
        ]);
      } else {
        await SunmiPrinter.printText(
          languages.customer,
        ...
        await SunmiPrinter.printText(
          ...
          style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT),
        );
      }
      await SunmiPrinter.lineWrap(1);
      if (orderDetailsPojo.userTakenType != 2) {
        await SunmiPrinter.printText(orderDetailsPojo.deliveryAddress, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT));
      } else {
        await SunmiPrinter.printText(languages.takeaway, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.CENTER));
      }

      if ((orderDetailsPojo.additionalRemark).isNotEmpty) {
        await SunmiPrinter.printText(languages.instruction, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true));
        await SunmiPrinter.printText(orderDetailsPojo.additionalRemark, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT));
      }
      ...
      await SunmiPrinter.printRow(cols: [
        SunmiColumn(text: languages.navProducts, width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true)),
        SunmiColumn(text: languages.price, width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.CENTER, bold: true)),
        SunmiColumn(text: languages.total, width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT, bold: true)),
      ]);
      // print items with MD style (non-bold)
      for (var item in orderDetailsPojo.itemList) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: item.productName, width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: ' ', width: 1, style: SunmiTextStyle(fontSize: _fsMD)),
          SunmiColumn(text: '${item.quantity}x${getAmountWithCurrency(item.priceForOne)}', width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.CENTER)),
          SunmiColumn(text: ' ', width: 1, style: SunmiTextStyle(fontSize: _fsMD)),
          SunmiColumn(text: getAmountWithCurrency(item.priceForOne), width: 8, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }
      ...
      await SunmiPrinter.line();
      await SunmiPrinter.printRow(cols: [
        SunmiColumn(text: languages.itemTotal, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
        SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderItemTotal), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
      ]);
      if (getDouble(orderDetailsPojo.orderDeliveryCost) > 0) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: languages.deliveryCharges, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderDeliveryCost), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }
      if (getDouble(orderDetailsPojo.orderPackagingCost) > 0) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: languages.packagingCharge, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderPackagingCost), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }

      if (getDouble(orderDetailsPojo.orderTax) > 0) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: languages.tax, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderTax), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }

      if (getDouble(orderDetailsPojo.promocodeDiscount) > 0) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: orderDetailsPojo.promocodeName, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.promocodeDiscount), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }
      if (getDouble(orderDetailsPojo.orderDiscountCost) > 0) {
        await SunmiPrinter.printRow(cols: [
          SunmiColumn(text: languages.discount, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
          SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderDiscountCost), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
        ]);
      }
      ...
      // Total row bold MD
      await SunmiPrinter.printRow(cols: [
        SunmiColumn(text: languages.total, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT, bold: true)),
        SunmiColumn(text: getAmountWithCurrency(orderDetailsPojo.orderTotalPay), width: 10, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT, bold: true)),
      ]);
      await SunmiPrinter.printRow(cols: [
        SunmiColumn(text: languages.paymentType, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
        SunmiColumn(text: getPaymentType(orderDetailsPojo.paymentType), width: 11, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT)),
      ]);
      await SunmiPrinter.printRow(cols: [
        SunmiColumn(text: languages.paymentSettlement, width: 20, style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.LEFT)),
        SunmiColumn(
          text: orderDetailsPojo.storePaySettleStatus == 1
              ? languages.settle
              : languages.unSettle,
          width: 11,
          style: SunmiTextStyle(fontSize: _fsMD, align: SunmiPrintAlign.RIGHT),
        ),
      ]);
